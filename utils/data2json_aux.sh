...
Usage: $0 <data-dir> <dict> <aux_dict>
...
if [ -n "${lexicon}" ]; then
    suffix=_aux
    awk 'NR == FNR{w=$1; $1=""; a[w]=substr($0,2); next}; {printf $1" "; for (i=2; i<=NF; ++i) printf "%s%s", $i in a? a[$i]: "<unk>", i == NF? "\n": " "}' \
	${lexicon} ${text} > ${tmpdir}/output/token${suffix}.scp
    < ${tmpdir}/output/token${suffix}.scp utils/sym2int.pl --map-oov ${oov} -f 2- ${aux_dic} > ${tmpdir}/output/tokenid${suffix}.scp
    vocsize=$(tail -n 1 ${aux_dic} | awk '{print $2}')
    odim=$(echo "$vocsize + 2" | bc)
    < ${tmpdir}/output/tokenid${suffix}.scp awk -v odim=${odim} '{print $1 " " NF-1 "," odim}' > ${tmpdir}/output/shape${suffix}.scp

    cp ${tmpdir}/output/text.scp ${tmpdir}/output/text${suffix}.scp
    output_strs=${output_strs}"--output-scps text:${tmpdir}/output/text${suffix}.scp \
                             token:${tmpdir}/output/token${suffix}.scp \
                             tokenid:${tmpdir}/output/tokenid${suffix}.scp \
                             shape:${tmpdir}/output/shape${suffix}.scp:shape "
else
    echo "Script is only used for network with auxiliary phone output, you're missing a lexicon."
    exit 1;
fi
