...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        if self.ngpu == 0:
            ...
            # apex does not support torch.nn.DataParallel
            loss = (
                data_parallel(self.model, x, range(
        ...
        if self.use_apex:
            ...
            opt = optimizer.optimizer if hasattr(
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        if self.use_lang_tag:
            ...
            # lang_tens = lang_tags[0].new(n_batch, 1).fill(self.ignore_id)
            # for i in range(n_batch):
            #     lang_tens[i, 0] = lang_tags[i]
            # lang_tags = lang_tens.to(device)
        ...
        aux_ys_pad = pad_list(
            [torch.from_numpy(y).long() for y in aux_ys], self.ignore_id
        ).to(device)

        if self.use_lang_tag:
            return xs_pad, ilens, ys_pad, aux_ys_pad, lang_tags
        else:
            return xs_pad, ilens, ys_pad, aux_ys_pad
...
class CustomConverterMulEnc(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        ys_pad = pad_list(
            [
                torch.from_numpy(np.array(y[0]) if isinstance(
                    y, tuple) else y).long()
                for y in ys
            ],
            self.ignore_id,
...
def train(args):
    ...
    aux_odim = int(valid_json[utts[0]]["output"][1]["shape"][-1])
    if hasattr(args, "decoder_mode") and args.decoder_mode == "maskctc":
        odim += 1  # for the <mask> token
        aux_odim += 1
    setattr(args, 'aux_odim', aux_odim)
    for i in range(args.num_encs):
        logging.info("stream{}: input dims : {}".format(i + 1, idim_list[i]))
    logging.info("#output dims: {} #aux output dims: {}".format(
        str(odim), str(aux_odim)))
    ...
    if args.rnnlm is not None:
        ...
        rnnlm = lm_pytorch.ClassifierWithState(
            lm_pytorch.RNNLM(len(args.char_list),
                             rnnlm_args.layer, rnnlm_args.unit)
        )
    ...
    with open(model_conf, "wb") as f:
        ...
        f.write(
            json.dumps(
                (idim_list[0] if args.num_encs ==
                 1 else idim_list, odim, vars(args)),
                indent=4,
                ensure_ascii=False,
                sort_keys=True,
            ).encode("utf_8")
    ...
    if args.num_encs == 1:
        converter = CustomConverter(
            subsampling_factor=model.subsample[0], dtype=dtype, use_lang_tag=args.use_lang_tag)
    else:
    ...
    if use_sortagrad:
        trainer.extend(
            ...
            trigger=(args.sortagrad if args.sortagrad != -
                     1 else args.epochs, "epoch"),
        )
    ...
    if args.save_interval_iters > 0:
        trainer.extend(
            CustomEvaluator(model, {"main": valid_iter},
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/aux_acc", "validation/main/aux_acc"], "epoch", file_name="aux_acc.png"
        )
    )
    trainer.extend(
        extensions.PlotReport(
    ...
    trainer.extend(extensions.ProgressBar(
        update_interval=args.report_interval_iters))
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    se2e = WindowStreamingE2E(
                        e2e=model, recog_args=args, rnnlm=rnnlm)
...
def enhance(args):
    ...
    if args.enh_wspecifier is not None:
        enh_writer = file_writer_helper(
            args.enh_wspecifier, filetype=args.enh_filetype)
    else:
        enh_writer = None
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                feat = feat[0]
                label = label[0]
                enc = model.encode(torch.as_tensor(
                    feat).to(device)).unsqueeze(0)
