...
from espnet.nets.pytorch_backend.e2e_asr_auxtask import Reporter
...
from espnet.nets.pytorch_backend.transformer.decoder_auxtask import Decoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument(
            ...
            type=int,
            default="0",
            help="output dimension of secondary output",
        )
        group.add_argument(
            "--transformer-init",
            type=str,
        ...
        group.add_argument(
            "--auxdlayer", default=1, type=int, help="Layer from where the auxiliary output comes from"
        )
        group.add_argument(
            "--auxbeta",
            default=0.0,
            type=float,
            help="Auxiliary task learning coefficient, beta: \
                            beta*aux_loss + (1-beta)*att_loss",
        )
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.mtlalpha < 1:
            ...
            self.aux_criterion = LabelSmoothingLoss(
                aux_odim,
                ignore_id,
                args.lsm_weight,
                args.transformer_length_normalized_loss,
        ...
        if self.decoder_mode == "maskctc":
            ...
            self.sos = odim - 2
            self.eos = odim - 2
            self.aux_mask_token = aux_odim - 1
            self.aux_sos = aux_odim - 2
            self.aux_eos = aux_odim - 2
        else:
            self.sos = odim - 1
            self.eos = odim - 1
            self.aux_sos = aux_odim - 1
            self.aux_eos = aux_odim - 1
        self.odim = odim
        self.aux_odim = aux_odim
    ...
    def forward(self, xs_pad, ilens, ys_pad, aux_ys_pad, *langs):
        ...
        if self.decoder is not None:
            if self.decoder_mode == "maskctc":
                ...
                aux_ys_in_pad, aux_ys_out_pad = mask_uniform(
                    aux_ys_pad, self.aux_mask_token, self.aux_eos, self.ignore_id
                )
                aux_ys_mask = (aux_ys_in_pad != self.ignore_id).unsqueeze(-2)
            else:
                ys_in_pad, ys_out_pad = add_sos_eos(
                    ys_pad, self.sos, self.eos, self.ignore_id
                )
                ys_mask = target_mask(ys_in_pad, self.ignore_id)
                aux_ys_in_pad, aux_ys_out_pad = add_sos_eos(
                    aux_ys_pad, self.aux_sos, self.aux_eos, self.ignore_id
                )
                aux_ys_mask = target_mask(aux_ys_in_pad, self.ignore_id)
            pred_pad, pred_mask, aux_pred_pad, aux_pred_mask = self.decoder(
                ys_in_pad, ys_mask, hs_pad, hs_mask, aux_ys_in_pad, aux_ys_mask)
            self.pred_pad = pred_pad
            self.aux_pred_pad = aux_pred_pad
            # 3. compute attention loss
            loss_att = self.criterion(pred_pad, ys_out_pad)
            self.acc = th_accuracy(
                pred_pad.view(-1, self.odim), ys_out_pad, ignore_label=self.ignore_id
            )
            aux_loss = self.aux_criterion(aux_pred_pad, aux_ys_out_pad)
            self.aux_acc = th_accuracy(
                aux_pred_pad.view(-1, self.aux_odim), aux_ys_out_pad, ignore_label=self.ignore_id
            )
        else:
            loss_att = None
            self.acc = None
        ...
        if self.training or self.error_calculator is None or self.decoder is None:
            cer, wer = None, None
            aux_cer, aux_wer = None, None
        else:
            ys_hat = pred_pad.argmax(dim=-1)
            cer, wer = self.error_calculator(ys_hat.cpu(), ys_pad.cpu())
            aux_ys_hat = aux_pred_pad.argmax(dim=-1)
            aux_cer, aux_wer = self.error_calculator(
                aux_ys_hat.cpu(), aux_ys_pad.cpu())

        # copied from e2e_asr
        alpha = self.mtlalpha
        beta = self.auxbeta
        if alpha == 0:
            self.loss = (1 - beta) * loss_att + beta * aux_loss
            loss_att_data = float(loss_att)
            loss_aux_data = float(aux_loss)
            loss_ctc_data = None
        elif alpha == 1:
            self.loss = loss_ctc
            loss_att_data = None
            ...
            self.loss = alpha * loss_ctc + \
                (1 - alpha - beta) * loss_att + beta * aux_loss
            loss_att_data = float(loss_att)
            loss_aux_data = float(aux_loss)
            loss_ctc_data = float(loss_ctc)
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ctc_data, loss_att_data, loss_aux_data, self.acc, self.aux_acc,
                cer_ctc, cer, wer, aux_cer, aux_wer, loss_data
            )
        else:
    ...
    def recognize_maskctc(self, x, recog_args, char_list=None, *langs):
        ...
        y_in = torch.zeros(1, len(y_idx) + 1,
                           dtype=torch.long) + self.mask_token
