...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_att, loss_aux, acc, aux_acc, cer_ctc, cer, wer, aux_cer, aux_wer, mtl_loss):
        """Report at every step."""
        reporter.report({"loss_ctc": loss_ctc}, self)
        reporter.report({"loss_att": loss_att}, self)
        reporter.report({"loss_aux": loss_aux}, self)
        reporter.report({"acc": acc}, self)
        reporter.report({"aux_acc": aux_acc}, self)
        reporter.report({"cer_ctc": cer_ctc}, self)
        reporter.report({"cer": cer}, self)
        reporter.report({"wer": wer}, self)
        reporter.report({"aux_cer": aux_cer}, self)
        reporter.report({"aux_wer": aux_wer}, self)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.mtlalpha == 0 or self.char_list is None:
            ...
            for i, y in enumerate(y_hats):
                ...
                seq_hat = [self.char_list[int(idx)]
                           for idx in y_hat if int(idx) != -1]
                seq_true = [
                ...
                if len(ref_chars) > 0:
                    cers.append(
                        editdistance.eval(
                            hyp_chars, ref_chars) / len(ref_chars)
                    )
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad):
        ...
        with torch.no_grad():
            # 0. Frontend
            if self.frontend is not None:
                hs_pad, hlens, mask = self.frontend(
                    to_torch_tensor(xs_pad), ilens)
                hs_pad, hlens = self.feature_transform(hs_pad, hlens)
            else:
                hs_pad, hlens = xs_pad, ilens
